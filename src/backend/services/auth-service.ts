...
export class AuthService {
  ...
  async signupUser(payload: SignupPayload) {
    ...
    if (authError || !authData.user) {
      ...
      throw new AppError({
        message: authError?.message || "Failed to sign up user in auth schema",
    ...
    if (insertProfileError) {
      console.error(
        "Error inserting user profile into public.users:",
        insertProfileError,
      );
      // Consider cleanup of auth user if profile insertion fails critically
      throw new AppError({
        message:
          insertProfileError.message ||
          "User authenticated but failed to create profile.",
        statusCode: 500,
      });
    }

    console.log(
      "User signed up and profile created successfully for:",
      user.email,
    );

    // Step 3: Provision Wallet based on role
    try {
      if (role === "sponsor") {
        const wallet = await this.walletService.provisionSponsorWallet(user.id);
        if (!wallet) {
          console.warn(
            `Sponsor wallet provisioning may have failed for user ${user.id} but signup continues.`,
          );
        }
      } else if (role === "vendor") {
        const wallet = await this.walletService.provisionVendorWallet(user.id);
        if (!wallet) {
          console.warn(
            `Vendor wallet provisioning may have failed for user ${user.id} but signup continues.`,
          );
        }
      }
    } catch (walletError) {
      // It's good practice to check the type of error before accessing properties
      let errorMessage =
        "An unknown error occurred during wallet provisioning.";
      if (walletError instanceof Error) {
        errorMessage = walletError.message;
      }
      console.error(
        `Failed to provision wallet for user ${user.id} during signup:`,
        errorMessage,
      );
      // Decide if this is a critical failure. For now, signup proceeds.
    }

    return { user }; // Return AuthUser object as per Supabase
  }
  ...
  async getUserBySession(sessionToken: string): Promise<User | null> {
    ...
    if (error || !data || !data.user) {
      if (error && error.status !== 401) {
        // Log unexpected errors (401 is expected for invalid token)
    ...
    try {
      ...
      if (!profile) {
        ...
        console.error(
          `Profile not found in public.users for valid auth user: ${authUser.id}`,
        );
        return null;
      }
      return profile;
      ...
      if (profileError instanceof AppError) {
        console.error(
          "AppError fetching profile for valid session:",
          profileError.message,
          profileError.statusCode,
        );
      } else {
        console.error(
          "Error fetching profile for valid session:",
          profileError,
        );
      }
