...
import { SmartWalletService } from "./smartWalletService";
  ...
  static async signUp(
    ...
    try {
      ...
      if (authError || !authData?.user) {
        console.log(
          "Standard signup failed, attempting admin API signup",
          authError,
        );
      ...
      try {
        if (role === "sponsor") {
          ...
          await SmartWalletService.createSmartWalletForVendor(authData.user.id);
        }
      } catch (error) {
