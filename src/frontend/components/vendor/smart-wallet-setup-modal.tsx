...
import {
  useVendorSetupStore,
  useVendorSetupActions,
} from "@/frontend/store/vendor-setup-store";
...
const setupSteps = [
  {
    ...
    description: "Create or connect your Smart Wallet via Coinbase.",
  },
  {
    title: "Register Wallet",
    description: "Confirm and register this wallet with ZepPay.",
  },
];
...
export const SmartWalletSetupModal = () => {
  ...
  const generatedAddress = useVendorSetupStore(
    (state) => state.generatedWalletAddress,
  );
  ...
  const { mutate: registerWallet, isPending: isRegistering } =
    useRegisterVendorWalletMutation();
  ...
  const handleConnectWallet = () => {
    ...
    if (coinbaseConnector) {
      ...
      console.warn(
        "Direct cbWalletConnector used as specific Coinbase connector was not found in list. Ensure cbWalletConnector is specific.",
      );
    }
  };
  ...
  const handleRegisterWallet = () => {
    if (generatedAddress) {
      ...
      toast({
        title: "Error",
        description: "No wallet address to register.",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
        ...
        <ModalBody pb={6}>
          ...
          {currentStep === 0 && (
            <VStack spacing={4} align="center" textAlign="center" p={4}>
              ...
              <Text>
                To receive payments on ZepPay, you need a Smart Wallet.
              </Text>
              <Text>
                We&apos;ll help you create or connect one securely using
                Coinbase.
              </Text>
              <Button
                colorScheme="brand"
              ...
              {isConnecting && (
                <Text fontSize="sm" color="gray.500" mt={2}>
                  Please follow the prompts from Coinbase Wallet...
                </Text>
              )}
              {localConnectError && (
                <Text color="red.500" mt={2} fontWeight="medium">
                  Error: {localConnectError}
                </Text>
              )}
            </VStack>
          )}
            ...
            <VStack spacing={4} align="stretch" p={4}>
              ...
              <Box borderWidth="1px" borderRadius="md" p={3} bg="gray.50">
                ...
                <Text
                  ...
                  maxWidth="100%"
                >
                  {generatedAddress || "Fetching address..."}
                </Text>
              </Box>
              <Text mt={2}>
                Please confirm this is the correct wallet address you wish to
