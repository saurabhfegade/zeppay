...
import { UserRole, WaasWallet } from "../../../common/types/database.types";
...
export default async function handler(
  ...
  try {
    ...
    const data = await validateRequestBody<SignupRequest>(
      SignupSchema,
      req,
      res,
    );
    ...
    try {
      if (data.role === "sponsor") {
        ...
        sponsorWallet = await WalletService.createWaasWalletForSponsor(
          authData.user.id,
        );
        console.log("Sponsor wallet created successfully:", sponsorWallet);
      } else if (data.role === "vendor") {
        // For vendors, wallet setup is handled post-signup via frontend flow.
        // No wallet is created or registered at this stage.
        console.log(
          `Vendor ${authData.user.id} signed up. Wallet to be set up via frontend flow.`,
        );
      }
    } catch (error) {
