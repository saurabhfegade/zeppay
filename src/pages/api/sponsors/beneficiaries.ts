...
async function handler(req: AuthenticatedRequest, res: NextApiResponse) {
  ...
  if (!userId || !accessToken) {
    return handleError(
      new ApiError("User not authenticated or token missing", 401),
      res,
    );
  }
  ...
  const requestScopedSupabaseClient = createClient(
    supabaseUrl,
    supabaseAnonKey,
    {
      global: {
  ...
  const serviceSupabaseClient =
    requestScopedSupabaseClient as SupabaseClient<any>;
  ...
  try {
    ...
    if (req.method === "GET") {
      const beneficiaries = await beneficiaryService.getBeneficiariesForSponsor(
        userId,
      );
      ...
      const newBeneficiary = await beneficiaryService.addBeneficiaryToSponsor(
        userId,
