...
const VendorProfilePage: NextPage = () => {
  ...
  const { isLoading: isLoadingVendorCategories, error: errorVendorCategories } =
    useGetVendorCategoriesQuery();
  const assignedCategoriesFromStore = useVendorProfileStore(
      ...
      { category_ids: [selectedCategoryId] },
      {
        ...
        onError: (error) => {
          toast({
            title: "Update failed",
            description:
              error.message || "Could not update category. Please try again.",
            status: "error",
            duration: 5000,
            isClosable: true,
          });
  ...
  if (errorAllCategories || errorVendorCategories) {
    return (
      <Alert status="error">
        ...
        {errorAllCategories && (
          <Text fontSize="sm">{errorAllCategories.message}</Text>
        )}
        {errorVendorCategories && (
          <Text fontSize="sm">{errorVendorCategories.message}</Text>
        )}
      </Alert>
    );
  }
    ...
    <Box p={8}>
      <VStack spacing={6} align="stretch">
        ...
        <Box p={6} borderWidth="1px" borderRadius="lg" shadow="md">
          ...
          {assignedCategoriesFromStore &&
            assignedCategoriesFromStore.length > 0 && (
              <Alert status="info" mb={4} colorScheme="brand">
                <AlertIcon />
          ...
          {allCategories && allCategories.length > 0 ? (
            ...
            <Text>
              No categories available to select. Please contact support.
            </Text>
          )}
        </Box>
      </VStack>
